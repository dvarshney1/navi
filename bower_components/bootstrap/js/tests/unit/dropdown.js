$(function() {
  ...
  QUnit.test('should ignore space key events for <input>s within dropdown, and accept up, down and escape', function(
    assert
  ) {
    assert.expect(6);
    var done = assert.async();
  ...
  QUnit.test('should ignore space key events for <textarea>s within dropdown, and accept up, down and escape', function(
    assert
  ) {
    assert.expect(6);
    var done = assert.async();
  ...
  QUnit.test('should not stop key event propagation when dropdown is disabled', function(assert) {
    ...
    // Key escape
    $dropdown.trigger('focus').trigger(
      $.Event('keydown', {
        which: 27
      })
    );
