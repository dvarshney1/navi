$(function() {
  ...
  QUnit.test('should remove active class on page load from button checkbox without checked attribute', function(
    assert
  ) {
    assert.expect(1);
    var done = assert.async();
  ...
  QUnit.test('should not do anything if the click was just sent to the outer container with data-toggle', function(
    assert
  ) {
    assert.expect(4);
    var groupHTML =
  ...
  QUnit.test('should not try and set checked property on an input that is not a radio button or checkbox', function(
    assert
  ) {
    assert.expect(2);
    var groupHTML =
