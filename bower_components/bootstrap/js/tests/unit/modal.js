$(function() {
  ...
  QUnit.test('should not adjust the inline body padding when it does not overflow, even on a scaled display', function(
    assert
  ) {
    assert.expect(1);
    var done = assert.async();
  ...
  QUnit.test('should scroll to top of the modal body if the modal has .modal-dialog-scrollable class', function(
    assert
  ) {
    assert.expect(2);
    var done = assert.async();
  ...
  QUnit.test("should set .modal's scroll top to 0 if .modal-dialog-scrollable and modal body do not exists", function(
    assert
  ) {
    assert.expect(1);
    var done = assert.async();
  ...
  QUnit.test('should close modal when escape key is pressed with keyboard = true and backdrop is static', function(
    assert
  ) {
    assert.expect(1);
    var done = assert.async();
  ...
  QUnit.test('should not close modal when escape key is pressed with keyboard = false and backdrop = static', function(
    assert
  ) {
    assert.expect(1);
    var done = assert.async();
    var $modal = $(
      '<div class="modal" data-backdrop="static" data-keyboard="false"><div class="modal-dialog" /></div>'
    ).appendTo('#qunit-fixture');
